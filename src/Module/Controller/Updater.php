    ...
    public function installAction($request)
    {
        ...
        try
        {
            ...
            if($updater->getType() == 'module')
            {
                ...
                $this->log(sprintf($this->t('packageUpdateProcessLogModule'), $updater->getName(), $updater->getVersion()), 2, 'Package');
            }
            else
            {
                $this->log(sprintf($this->t('packageUpdateProcessLogCore'), $updater->getVersion()), 2, 'Package');
            }
        }
        catch(\Exception $e)
        {
